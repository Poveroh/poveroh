...
import {
    AccountSummaryRangeQuerySchema,
    ...
    GetAccountSummaryResponseSchema,
...
export const registerFinancialAccountBalancePath = (registry: OpenAPIRegistry) => {
    ...
    registry.registerPath({
        method: 'get',
        path: '/financial-accounts/{id}/summary',
        tags: ['Financial Account'],
        operationId: 'getFinancialAccountSummary',
        summary: 'Get a financial account period summary',
        description:
            'Retrieve the aggregated income, expenses and transaction count of a financial account within an optional date range',
        security: [{ bearerAuth: [] }],
        request: {
            params: FinancialAccountParamsId.describe('ID of the financial account whose summary to retrieve'),
            query: AccountSummaryRangeQuerySchema
        },
        responses: {
            200: {
                description: 'Financial account period summary',
                content: {
                    'application/json': {
                        schema: GetAccountSummaryResponseSchema
                    }
                }
            },
            400: {

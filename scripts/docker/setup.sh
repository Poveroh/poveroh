...
fetch_file_from_github() {
    ...
    if [[ $? -ne 0 ]]; then
        echo "Error downloading $file_path from GitHub."
        exit 1
    fi
}
...
start_images() {
    ...
    docker network ls | grep -w poveroh_network || docker network create poveroh_network
    ...
    if [[ -z "$database_host" ]]; then
        echo "DATABASE_HOST is not set in $ENV_FILE."
        exit 1
    fi
